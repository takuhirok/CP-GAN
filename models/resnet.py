...
class Generator(nn.Module):
    ...
    def forward(self, input, label=None, class_weight=None):
        ...
        output = output.view(-1, self.channels, self.image_size // 8,
                             self.image_size // 8)
        output = self.block2(output, label=label, class_weight=class_weight)
        output = self.block3(output, label=label, class_weight=class_weight)
        output = self.block4(output, label=label, class_weight=class_weight)
